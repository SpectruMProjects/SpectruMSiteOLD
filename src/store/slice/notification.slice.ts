...
import {fetchRegistrationAccount} from "../thunk"
...
export const notificationSlice = createSlice({
    ...
    reducers: {
        ...
        actionDeleteError: (state: Draft<INotificationState>, action: PayloadAction<string>) => {
            state.errors = state.errors.filter(
                (error: Draft<IError>) => error.id !== action.payload
            );
        },
        ...
        actionDeleteAction: (state: Draft<INotificationState>, action: PayloadAction<string>) => {
            state.action = state.action.filter(
                (value: Draft<IAction>) => value.id !== action.payload
            );
        },
    },
    extraReducers(builder) {
        builder
            ...
            .addCase(fetchRegistrationAccount.fulfilled, (state: Draft<INotificationState>, action: PayloadAction<IError | void>) => {
